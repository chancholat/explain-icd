...
LRS=(5e-5 1e-5)                          # $1 -> 0..1
# LAMBDAS=(0.0001 0.00001 0.000001 0.001)  # $2 -> 0..3
LAMBDAS=(0)
...
echo "Fixed: EXPERIMENT=${EXPERIMENT}, BATCH=${BATCH}, GPU=${GPU}  ${#LAMBDAS[@]}"
echo "Will sweep: lambdas × ${#METHODS[@]} methods × ${#REFS[@]} refs"
...
for REF in "${REFS[@]}"; do
  for METHOD in "${METHODS[@]}"; do
    for LAMBDA in "${LAMBDAS[@]}"; do
      ...
      TAG="lr${LR}_lam${LAMBDA}_${METHOD}_${REF_BASENAME}"
